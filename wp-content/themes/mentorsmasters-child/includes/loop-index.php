	...
	echo "<article class='".implode(" ", get_post_class('post-entry post-entry-type-'.$post_format . " " . $post_class . " ".$with_slider))."' ".avia_markup_helper(array('context' => 'entry','echo'=>false)).">";
        ...
        echo "<div class='entry-content-wrapper clearfix {$post_format}-content'>";
            echo '<header class="entry-content-header">';
				...
				$_starttime_ = date( "F j, Y", strtotime( get_post_meta( get_the_ID(), '_goo_cmevent_start', true ) ));
				$_endtime_ = date( "F j, Y", strtotime( get_post_meta( get_the_ID(), '_goo_cmevent_end', true ) ));
				
				$markup = avia_markup_helper(array('context' => 'entry_time','echo'=>false));
				if( get_post_type() === 'cmevents' ){
					echo "<span class='_goo-archive-date'>" . $_starttime_ . " - " . $_endtime_ . "</span>";
			...
			$_venue_ = get_post_meta( get_the_ID(), '_goo_cmevent_address_venue', true );
			$_street_ = get_post_meta( get_the_ID(), '_goo_cmevent_address_street', true );
			$_city_ = get_post_meta( get_the_ID(), '_goo_cmevent_address_city', true );
			$_state_ = get_post_meta( get_the_ID(), '_goo_cmevent_address_state', true );
			$_zip_ = get_post_meta( get_the_ID(), '_goo_cmevent_address_zip', true );
			$_country_ = get_post_meta( get_the_ID(), '_goo_cmevent_address_country', true );
			
			$_address_ = array( $_city_, $_state_, $_zip_, $_country_ );
			
			$itr = 0;
			foreach( $_address_ as $val ){
				if( $val === NULL ){
					array_splice( $_address_, $itr, 1 );
				}
				$itr++;
			}
			$_address_ = implode( ", ", $_address_ );
			
			echo "<div class='_goo-archive-address'>";
			echo "<ul>";
			echo "<li class='venue'>$_venue_</li>";
			echo "<li class='street'>$_street_</li>";
			echo "<li class='city-state'>$_address_</li>";
			echo "</ul>";
			echo "</div>";
