                ...
                foreach ( $listings as &$post ) {
                    ...
                    if ( $show_images ) {
                        if ( $img_id = $listing->get_thumbnail_id() ) {
                            $out .= '<div class="goo-feat-img"><a href="' . get_permalink( $post->ID ) . '">' . wp_get_attachment_image( $img_id, $img_size, false, array( 'class' => 'listing-image' ) ) . '</a></div>';
                        }
                    }
                    $out.= "<div class='goo-feat-excerpt'><h4>".get_the_title( $post->ID )."</h4><p>". get_the_excerpt() ."</p><a href='".get_permalink( $post->ID )."' class='read-more'>Read More</a></div>";
