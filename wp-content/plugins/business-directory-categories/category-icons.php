...
class WPBDP_CategoryIconsModule {
    ...
    public function register_settings( &$api ) {
        ...
        $api->add_setting( $s, 'categories-use-images', __( 'Display the category list using images', 'wpbdp-categories' ), 'boolean', false );
     ...
     * Category images.
     */
    private function register_temp_image($term_id, $image_path, $image_url) {
        $images = $this->get_temp_images($term_id);
        $images[] = array('path' => $image_path, 'url' => $image_url);
    ...
    private function get_term_image($term_id) {
        $category_images = get_option('wpbdp[category_images]');
        return isset($category_images['images'][$term_id]) ? $category_images['images'][$term_id] : null;
    }

    private function set_term_image($term_id, $image_path, $image_url, $do_cleanup=true) {
        $temp_images = $this->get_temp_images($term_id);
        if ($current_image = $this->get_term_image($term_id)) {
            if ($current_image['path'] != $image_path)
        ...
        if ( file_exists( $image_path ) && ! empty( $image_url ) ) {
            $category_images['images'][$term_id] = array( 'path' => $image_path, 'url' => $image_url );
        } else {
...
EOT;
        ...
        if (isset($_FILES['image_upload']) && $_FILES['image_upload']['error'] == 0) {
            ...
            if ($upload = wp_handle_upload($_FILES['image_upload'], array('test_form' => FALSE))) {
                if (!isset($upload['error'])) {
                    ...
                    $this->register_temp_image($_GET['term_id'], $upload['file'], $upload['url']);
    ...
    public function _category_update($term_id, $tt_id, $taxonomy) {
        if (isset($_POST['category_image'])) {
            ...
            if (!empty($path) && !empty($url)) {
                $this->set_term_image($term_id, $path, $url, true);
            } else {
                $this->set_term_image($term_id, null, null, true);
            }
        }
    }
    ...
    public function _categories_list_item($item_html, $term) {
        ...
        if ($image) {
            $image_html .= sprintf('<a href="%s"><img src="%s" width="%d" height="%d" class="category-image" /></a>',
